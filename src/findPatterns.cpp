...
Rcpp::List findPatterns(IntegerVector PrePro_indexFilter,
    ...
    // Initialize output containers with reserved capacity
    // OPTIMIZATION: Pre-allocate memory to avoid expensive reallocations during processing
    ...
    std::vector<double> trendBeginPrices, trendEndPrices;
    std::vector<int> trendBeginTimes, trendEndTimes;
    std::vector<double> returns1, returns3, returns5, returns10, returns30, returns60;
    std::vector<double> relReturns13, relReturns12, relReturns1, relReturns2, relReturns4;
    
    // Reserve memory based on maximum possible pattern count
    patternNames.reserve(maxPossiblePatterns);
    validPatterns.reserve(maxPossiblePatterns);
    firstIndexPrePro.reserve(maxPossiblePatterns);
    firstIndexOriginal.reserve(maxPossiblePatterns);
    breakoutIndices.reserve(maxPossiblePatterns);
    
    timeStamp0.reserve(maxPossiblePatterns);
    timeStamp1.reserve(maxPossiblePatterns);
    timeStamp2.reserve(maxPossiblePatterns);
    timeStamp3.reserve(maxPossiblePatterns);
    timeStamp4.reserve(maxPossiblePatterns);
    timeStamp5.reserve(maxPossiblePatterns);
    timeStampBreakout.reserve(maxPossiblePatterns);
    
    priceStamp0.reserve(maxPossiblePatterns);
    priceStamp1.reserve(maxPossiblePatterns);
    priceStamp2.reserve(maxPossiblePatterns);
    priceStamp3.reserve(maxPossiblePatterns);
    priceStamp4.reserve(maxPossiblePatterns);
    priceStamp5.reserve(maxPossiblePatterns);
    priceStampBreakout.reserve(maxPossiblePatterns);
    
    trendBeginPrices.reserve(maxPossiblePatterns);
    trendBeginTimes.reserve(maxPossiblePatterns);
    trendEndPrices.reserve(maxPossiblePatterns);
    trendEndTimes.reserve(maxPossiblePatterns);
    
    returns1.reserve(maxPossiblePatterns);
    returns3.reserve(maxPossiblePatterns);
    returns5.reserve(maxPossiblePatterns);
    returns10.reserve(maxPossiblePatterns);
    returns30.reserve(maxPossiblePatterns);
    returns60.reserve(maxPossiblePatterns);
    
    relReturns13.reserve(maxPossiblePatterns);
    relReturns12.reserve(maxPossiblePatterns);
    relReturns1.reserve(maxPossiblePatterns);
